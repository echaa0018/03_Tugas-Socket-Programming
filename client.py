...
def authentication():  # checks if the user knows the password
...
def receive():  # continuously receive messages from the server
    while True:
        try:
            ...
            print(f"\033[93m{decrypted_message}\033[0m")  # Corrected to print decrypted_message
            print()
...
while True:  # Loop for receiving and sending messages
    ...
    if message == "!q":  # client quits the chatroom
        ...
        client.sendto(encrypted_message.encode(), (server_ip, server_port))
